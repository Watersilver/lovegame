...
local onReturnListenersDefaults = {
  [false] = function(self)
...
onReturnListenersDefaults.ssbInvalidState = onReturnListenersDefaults.ssbFar
...
DialogueControl.functions = {
  ...
  handleHookReturn = function (self)
    if self.onHandleHookReturnStart then
      self:onHandleHookReturnStart()
    end
    local hr = self.hookReturn or false
    if self.onHookReturnListeners[hr] then
      self.onHookReturnListeners[hr](self)
    elseif onReturnListenersDefaults[hr] then
      onReturnListenersDefaults[hr](self)
    end
  end,
...
function DialogueControl:new(init)
  local instance = p:new() -- add parent functions and fields
  p.new(DialogueControl, instance, init) -- add own functions and fields
  return instance
end
